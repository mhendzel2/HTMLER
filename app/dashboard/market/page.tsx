    ...
    try {
      ...
      if (response.ok) {
        ...
        const transformedData: HottestChain[] = (data.data || []).map((chain: any) => ({
          ...
          volume: chain.volume ?? 0,
          open_interest: chain.open_interest ?? 0,
          premium: chain.premium ?? 0,
          strike: Number(chain.strike) || 0,
          expiry: chain.expiry || '—',
          type: chain.type?.toLowerCase() || 'call',
          underlying_price: chain.underlying_price ?? 0,
